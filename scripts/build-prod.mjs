/**
 * Production-сборка (корень домена, без basePath).
 * WebHOST1, REG.RU и любой Apache/nginx-хостинг со статикой.
 *
 *   npm run build:prod
...
console.log(`build:prod → SITE_URL=${siteUrl}, BASE_PATH=(корень)\n`);

for (const script of ["scripts/generate-favicon.mjs", "scripts/generate-short-redirects.mjs"]) {
